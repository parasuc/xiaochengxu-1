...
Page({
  ...
  init:function(){
     ...
     this.getBuy(page, this.data.catid, this.data.arrchildid, this.data.keyword);
...
//获取市
  ...
  //获取资讯
   getBuy: function (page, catid, areaid,keyword){
    var that=this;

    data.getBuy(function(data){
       var arr=[];
       for(var i=0;i<data.data.length;i++){
          arr.push({ title: data.data[i].title, areaname: data.data[i].areaname, catname:data.data[i].catname,company: data.data[i].company, edittime: new Date(data.data[i].edittime * 1000).toLocaleString().replace(/:\d{1,2}$/, ' ')})
       }
        that.setData({
           zixun: arr,
